...
use rust_gpt_2_impl::{
    ...
    llmc::tokenizer::Tokenizer,
    llmc::utils::{fopen_check, write_f32s, write_i32_header, write_u16s, write_u32s},
    {
        GPT2, GPT2Config, NUM_PARAMETER_TENSORS, fill_in_parameter_sizes, matmul_forward,
...
fn write_tokenizer_file(path: &std::path::Path, vocab: u32, eot: u32, tokens: &[&str]) {
    ...
    for tok in tokens {
        ...
        rust_gpt_2_impl::llmc::utils::fwrite_check(tok.as_bytes(), &mut f);
    }
}
...
fn finite_difference_gradient_check() {
    ...
    let inputs: Vec<i32> = (0..B * T)
        .map(|_| (randint32(&mut rng) % V as u32) as i32)
        .collect();
    ...
    assert!(
        (expected_loss - (V as f32).ln()).abs() < 0.5,
        "unexpected initial loss {expected_loss}"
    );
    ...
    println!(
        "finite-difference check: {checked}/{num_samples} params compared, max rel err {max_rel_err:e}"
    );
