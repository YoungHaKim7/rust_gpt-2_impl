...
use gpt2::llmc::tokenizer::{safe_printf, Tokenizer};
use gpt2::GPT2;
...
fn main() {
    ...
    let train_tokens = if Path::new(tiny_shakespeare_train).exists() { tiny_shakespeare_train } else { tiny_stories_train };
    let val_tokens = if Path::new(tiny_shakespeare_val).exists() { tiny_shakespeare_val } else { tiny_stories_val };
    ...
    println!("val dataset num_batches: {}", val_loader.num_tokens / (B * T));
    ...
    for step in 0..=40usize {
        ...
        model.gpt2_backward();
        model.gpt2_update(1e-4f32, 0.9f32, 0.999f32, 1e-8f32, 0.0f32, (step + 1) as i32);
        let time_elapsed_s = start.elapsed().as_secs_f64();
        println!("step {}: train loss {:.6} (took {:.6} ms)", step, model.mean_loss, time_elapsed_s * 1000.0);
    }
