...
use gpt2::llmc::utils::{read_f32s, read_i32s};
use gpt2::{malloc_and_point_parameters, GPT2};
...
fn main() {
    ...
    let (mut expected_grads_memory, expected_grads) = malloc_and_point_parameters(&model.param_sizes);
    ...
    for step in 0..10usize {
        ...
        if step == 0 {
            ...
            let calculated_logits =
                model.acts.unwrap().logits.slice(model.acts_memory.as_ref().unwrap(), 0, B * T * Vp);
            ...
            'outer: for bt in 0..B * T {
                for v in 0..V {
                    ...
                    if diff >= 1e-2f32 {
                        ...
                        println!("{:.6} {:.6}", expected_logits[bt * V + v], calculated_logits[i]);
                        logits_ok = false;
            ...
            if (model.mean_loss - expected_loss[0]).abs() >= 1e-2 {
                println!("LOSS MISMATCH: {:.6} {:.6}", model.mean_loss, expected_loss[0]);
                allok = false;
            } else {
            ...
            let checks: [(&[f32], &[f32], &str); 16] = [
                (g.wte.slice(gm, 0, V * C), expected_grads.wte.slice(&expected_grads_memory, 0, V * C), "dwte"),
                (g.wpe.slice(gm, 0, maxT * C), expected_grads.wpe.slice(&expected_grads_memory, 0, maxT * C), "dwpe"),
                (g.ln1w.slice(gm, 0, L * C), expected_grads.ln1w.slice(&expected_grads_memory, 0, L * C), "dln1w"),
                (g.ln1b.slice(gm, 0, L * C), expected_grads.ln1b.slice(&expected_grads_memory, 0, L * C), "dln1b"),
                (g.qkvw.slice(gm, 0, L * 3 * C * C), expected_grads.qkvw.slice(&expected_grads_memory, 0, L * 3 * C * C), "dqkvw"),
                (g.qkvb.slice(gm, 0, L * 3 * C), expected_grads.qkvb.slice(&expected_grads_memory, 0, L * 3 * C), "dqkvb"),
                (g.attprojw.slice(gm, 0, L * C * C), expected_grads.attprojw.slice(&expected_grads_memory, 0, L * C * C), "dattprojw"),
                (g.attprojb.slice(gm, 0, L * C), expected_grads.attprojb.slice(&expected_grads_memory, 0, L * C), "dattprojb"),
                (g.ln2w.slice(gm, 0, L * C), expected_grads.ln2w.slice(&expected_grads_memory, 0, L * C), "dln2w"),
                (g.ln2b.slice(gm, 0, L * C), expected_grads.ln2b.slice(&expected_grads_memory, 0, L * C), "dln2b"),
                (g.fcw.slice(gm, 0, L * 4 * C * C), expected_grads.fcw.slice(&expected_grads_memory, 0, L * 4 * C * C), "dfcw"),
                (g.fcb.slice(gm, 0, L * 4 * C), expected_grads.fcb.slice(&expected_grads_memory, 0, L * 4 * C), "dfcb"),
                (
                    g.fcprojw.slice(gm, 0, L * C * 4 * C),
                    expected_grads.fcprojw.slice(&expected_grads_memory, 0, L * C * 4 * C),
                    "dfcprojw",
                ),
                (g.fcprojb.slice(gm, 0, L * C), expected_grads.fcprojb.slice(&expected_grads_memory, 0, L * C), "dfcprojb"),
                (g.lnfw.slice(gm, 0, C), expected_grads.lnfw.slice(&expected_grads_memory, 0, C), "dlnfw"),
                (g.lnfb.slice(gm, 0, C), expected_grads.lnfb.slice(&expected_grads_memory, 0, C), "dlnfb"),
            ];
        ...
        model.gpt2_update(1e-4f32, 0.9f32, 0.999f32, 1e-8f32, 0.01f32, (step + 1) as i32);
        ...
        println!("step {}: loss {:.6} (took {:.6} ms) OK = {}", step, model.mean_loss, time_elapsed_s * 1000.0, step_loss_ok as i32);
    }
