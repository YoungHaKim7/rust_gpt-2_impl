...
use gpt2::{fill_in_parameter_sizes, GPT2Config, NUM_PARAMETER_TENSORS};
...
fn write_tokens_file(path: &Path, ntok: usize, vocab: usize, rng: &mut Mt19937State) {
    ...
    let tokens: Vec<u16> = (0..ntok).map(|_| (randint32(rng) % vocab as u32) as u16).collect();
...
fn main() {
    ...
    std::fs::create_dir_all(out_dir.join("dev/data/tinyshakespeare")).expect("could not create output dirs");
    ...
    println!("wrote {} ({} params)", out_dir.join("gpt2_124M.bin").display(), num_parameters);
    ...
    for i in 0..VOCAB_SIZE {
        ...
        gpt2::llmc::utils::fwrite_check(bytes, &mut f);
    }
